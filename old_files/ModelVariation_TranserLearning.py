...
from CityDatasetPrepare import ValidationData
...
def train(duration, epochs, batch_size, netMax_load, loss_tracker,
    ...
    netMax.load_state_dict(torch.load(netMax_load))
    ...
    epoch = 0
    batch_diff = []
    val, test = int(batch_size / 10), int(batch_size / 10)
    losses, tmp_losses = [[], [], []], []
    while (learn_rate >= end_lr and tt.time() < time[0] + duration) or epoch < epochs:
        for i in range(batch_size + val + test):
            netMax.train()
            if i < batch_size:
                state = "train"
                original, gradient, target = produce_image(False, 15, salt_and_pepper_prob=0.75, dim=dim_input,
                                                           grad=edge_finder)
                image = forward(original, resize, dim_input).unsqueeze(0)
                prep_target = forward2(target, resize, dim_input).float().unsqueeze(0)
            elif i == batch_size:
                losses[0].append(np.mean(loss_tracker[-batch_size:]))
                state = "eval"
                netMax.eval()
                original, gradient, target = produce_image(False, 15, salt_and_pepper_prob=0.0, dim=dim_input,
                                                           grad=edge_finder)
                image = forward(original, resize, dim_input).unsqueeze(0)
                prep_target = forward2(target, resize, dim_input).float().unsqueeze(0)
            else:
                state = "eval"
                original, target = vd.forward(1)
                image, prep_target = original.unsqueeze(0), \
                                     target.float()
            ...
            if state == "train":
                ...
                if i < batch_size + val:
                    tmp_losses.append(criterion(out, prep_target).item())

                else:
                    if i == batch_size + val:
                        losses[1].append(np.mean(tmp_losses))
                        tmp_losses = []
                    elif i == batch_size + val + test - 1:
                        losses[2].append(np.mean(tmp_losses))
                        tmp_losses = []
                    else:
                        tmp_losses.append(criterion(out, prep_target).item())
            ...
            if True:
                ...
                if plot or save:
                    ...
                    ax[1, 0].imshow(target.transpose(1, 0).clip(0, 1), interpolation="nearest")
                    ax[1, 1].imshow(original.transpose(1, 0, 2).clip(0, 1), interpolation="nearest")
        ...
        if plot or save:
            ...
            ax[0].plot(np.arange(5, len(loss_tracker) - 4), [np.mean(loss_tracker[x - 5:x + 5])
                                                             for x in np.arange(5, len(loss_tracker) - 4)])
            ax[1].plot(range(len(loss_tracker[-3 * batch_size:])), loss_tracker[-3 * batch_size:])
            if plot:
                plt.show()
            if save:
    ...
    if True or save:
        ...
        ax[2, 0].plot(np.arange(5, len(loss_tracker) - 4), [np.mean(loss_tracker[x - 5:x + 5])
                                                            for x in np.arange(5, len(loss_tracker) - 4)])
        ax[2, 1].plot(range(len(losses[0])), losses[0], color="blue")
        ax[2, 1].plot(range(len(losses[0])), losses[1], color="red")
        ax[2, 1].plot(range(len(losses[0])), losses[2], color="grey")
        ax[2, 2].plot(range(len(lr_data)), lr_data)
        ax[2, 2].set_yscale('log')
        ax[0, 0].axis('off')
        ax[0, 1].axis('off')
