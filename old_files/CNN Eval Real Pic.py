...
# Specify a path
PATH = "load_state_dict.pt.psd.psd.psd"
...
if os.path.exists(PATH):
    netMax.load_state_dict(torch.load(PATH))
# model.eval()
...
image = plt.imread("61084_anime_scenery_city_street_at_night.jpg")
gradient = cv2.Laplacian(image.astype('float64'), cv2.CV_64F)[:, :, 0:3]
# print(np.histogram(gradient))
...
result = np.where(result > 0.015, 1, 0).transpose(1, 0).astype('int32')
plt.imshow(result)
plt.show()