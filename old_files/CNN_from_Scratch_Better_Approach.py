...
class Net(nn.Module):
    def __init__(self, poolfunc, unpoolfunc):
        ...
        self.poolQ2 = poolfunc((2, 2), return_indices=True)
        self.poolQ5 = poolfunc((5, 5), return_indices=True)
        #self.unpool = unpoolfunc((4, 3))
        self.unpoolQ2 = unpoolfunc((2, 2))
        self.unpoolQ5 = unpoolfunc((5, 5))
    ...
    def forward(self, x):
        ...
        # torch.Size([1, 54, 40, 40])
        x = self.unconv1(x)
        x = self.unpoolQ2(x, indices2)
        x = self.unconv2(x)
        x = self.unpoolQ5(x, indices1)
        x = self.unconv3(x)
