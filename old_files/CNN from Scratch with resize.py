...
save, load, load_temp = True, False, False
...
for epoch in range(epochs):
    for i in range(batch):
        original, gradient, target = produce_image(False, 15 + 2 * (epoch+1), salt_and_pepper_prob=0.00, dim=(400, 300, 4))
        cv2.resize(original, dsize=(300, 400), interpolation=cv2.INTER_CUBIC)
