...
class Net(nn.Module):
    ...
    def __init__(self, pool_function, unpool_function, kernel_size, dim_input, activation_function1,
        ...
        self.conv1 = nn.Conv2d(3, 6, kernel_size, padding=padding)
        self.conv2 = nn.Conv2d(6, 9, kernel_size, padding=padding)
        self.conv3 = nn.Conv2d(9, 12, kernel_size, padding=padding)
        self.conv4 = nn.Conv2d(12, 18, kernel_size, padding=padding)
        self.conv5 = nn.Conv2d(18, 24, kernel_size, padding=padding)
        self.conv6 = nn.Conv2d(24, 30, kernel_size, padding=padding)
        self.conv7 = nn.Conv2d(30, 36, kernel_size, padding=padding)
        self.conv8 = nn.Conv2d(36, 42, kernel_size, padding=padding)
        self.conv9 = nn.Conv2d(42, 48, kernel_size, padding=padding)

        self.unconv1 = nn.ConvTranspose2d(48, 42, 2, stride=1)
        self.unconv2 = nn.ConvTranspose2d(42, 36, 2, stride=1)
        self.unconv3 = nn.ConvTranspose2d(36, 30, 2, stride=1)
        self.unconv4 = nn.ConvTranspose2d(30, 24, 2, stride=1)
        self.unconv5 = nn.ConvTranspose2d(24, 18, 2, stride=1)
        self.unconv6 = nn.ConvTranspose2d(18, 12, 2, stride=1)
        self.unconv7 = nn.ConvTranspose2d(12, 9, 2, stride=1)
        self.unconv8 = nn.ConvTranspose2d(9, 6, 2, stride=1)
        self.unconv9 = nn.ConvTranspose2d(6, 3, 2, stride=1)
        self.unconv10 = nn.ConvTranspose2d(3, 1, 2, stride=1)
        ...
        self.flatten_neurons = int(Net.__dim_input__[0] * Net.__dim_input__[1] / 16)
        self.fc1 = nn.Linear(48 * self.flatten_neurons, 36 * self.flatten_neurons)
        self.fc2 = nn.Linear(36 * self.flatten_neurons, 24 * self.flatten_neurons)
        self.fc3 = nn.Linear(24 * self.flatten_neurons, 12 * self.flatten_neurons)
        self.fc4 = nn.Linear(12 * self.flatten_neurons, 24 * self.flatten_neurons)
        self.fc5 = nn.Linear(24 * self.flatten_neurons, 36 * self.flatten_neurons)
        self.fc6 = nn.Linear(36 * self.flatten_neurons, 48 * self.flatten_neurons)
    ...
    def forward(self, x):
        ...
        if self.pool_func_is_MaxPool:
            x, indices1 = self.poolQ2(F.relu(self.conv1(x)))
            x, indices2 = self.poolQ2(F.relu(self.conv2(x)))
        else:
            indices1, indices2 = None, None
            x = self.poolQ2(F.relu(self.conv1(x)))
        ...
        x = F.hardtanh(self.fc4(x))
        x = F.hardtanh(self.fc5(x))
        x = F.hardtanh(self.fc6(x))
        x = x.view(-1, 48, int(Net.__dim_input__[0] / 4), int(Net.__dim_input__[1] / 4))

        # unpooling and unconvolution section
        x = F.hardtanh(self.unconv1(x))
        x = F.hardtanh(self.unconv2(x))
        x = F.hardtanh(self.unconv3(x))
        x = F.hardtanh(self.unconv4(x))
        x = F.hardtanh(self.unconv5(x))
        x = F.hardtanh(self.unconv6(x))

        x = self.unpoolQ2(self.unconv7(x), indices2)
        x = self.activation_function2(self.unpoolQ2(self.unconv8(x), indices1))
        x = F.hardtanh(self.unconv9(x))
        if self.activation_function3 == torch.threshold:
            x = self.activation_function3(self.unconv10(x), threshold=1, value=0)
        elif self.activation_function3 == nn.Hardtanh:
            x = self.activation_function3(self.unconv10(x), min_val=0.0, max_val=0.0)
        else:
            x = self.activation_function3(self.unconv10(x))
...
def train(duration, epochs, batch_size,
    ...
    while (learn_rate >= end_lr and tt.time() < time[0] + duration) or epoch < epochs:
        ...
        if rek_batch_comparison(batch_diff, learn_rate, 4) and epoch > 4:
            learn_rate /= 10
            optimizer = optim.Adam(netMax.parameters(), lr=learn_rate)
        print(str(epoch)+" Epochen berechnet.")
    # Save
    if save:
