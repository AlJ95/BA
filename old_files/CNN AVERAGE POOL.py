...
class Net(nn.Module):
    def __init__(self):
        ...
        self.poolQ5 = nn.AvgPool2d((5, 5))
        self.poolQ2 = nn.AvgPool2d((2, 2))
    ...
    def forward(self, x):
        ...
        #x = self.poolQ(F.relu(self.conv3(x)))
        ...
        x = nn.functional.interpolate(x, [shapes[-1][2], shapes[-1][3]],mode="bicubic").clamp(min=0, max=255)
        x = self.unconv1(x)
        x = nn.functional.interpolate(x, [shapes[-2][2], shapes[-2][3]],mode="bicubic").clamp(min=0, max=255)
        x = self.unconv2(x)
        #x = nn.functional.interpolate(x, [shapes[-3][2], shapes[-3][3]])
        x = self.unconv3(x)
