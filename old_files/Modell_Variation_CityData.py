...
def train(duration, epochs, batch_size,
    ...
    while (learn_rate >= end_lr and tt.time() < time[0] + duration) or epoch < epochs:
        for i in range(batch_size):
            ...
            ##                                           grad=edge_finder)
            inp, target = vd.forward(1, True, dim_input)
            #image = forward(inp, resize, dim_input)
            ...
            if i == batch_size - 1:
                ...
                if plot or save:
                    inp = inp.numpy() - np.min(inp.numpy())
                    inp /= np.max(inp)
                    ...
                    ax[0, 0].imshow((image.transpose(1, 2, 0)-np.min(image))/np.max(image), cmap="seismic")
                    ...
                    ax[1, 0].imshow(target.squeeze(0), interpolation="nearest")
                    ax[1, 1].imshow(inp.transpose(1,2,0), interpolation="nearest")
