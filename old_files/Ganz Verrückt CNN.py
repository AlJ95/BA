...
def forward(x):
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (1 / 12, 1 / 12, 1 / 12))
    ])

    x = PIL.Image.fromarray(np.array(x * 255 / np.max(x)).astype('uint8'))
    x = transform(x)
...
class Net(nn.Module):
    def __init__(self, poolfunc, unpoolfunc):
        ...
        # self.poolQ = poolfunc((2, 2), return_indices=True)
        # self.unpool = unpoolfunc((4, 3))
        ##elf.unpoolQ = unpoolfunc((2, 2))

        # an affine operation: y = Wx + b
        self.fc1 = nn.Linear(36 * 30 ** 2, 10 * 30 ** 2)
        self.fc2 = nn.Linear(10 * 30 ** 2, 36 * 30 ** 2)
...
def train(duration, epochs, batch_size, plot=True, save=True, load=True, title="no title"):
    ...
    optimizer = optim.Adam(netMax.parameters(), lr=0.01)
    ...
    while (epoch < epochs and tt.time() < time[0] + duration) or epoch == 0:
        for i in range(batch_size):
            ...
            if plot and i == 0:
                ...
                ax[0, 1].imshow(out_numpy.transpose(1, 0), interpolation="nearest")
                ax[0, 2].imshow(np.where(out_numpy.transpose(1, 0) > 0.8, 255, 0).astype('int32'), interpolation="nearest")
        ...
        ax[0].plot(range(len(loss_tracker[2:])), loss_tracker[2:], color="red")
